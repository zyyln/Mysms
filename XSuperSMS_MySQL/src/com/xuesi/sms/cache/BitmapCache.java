...
import com.xuesi.mos.libs.volley.toolbox.ImageLoader.ImageCache;
...
public class BitmapCache implements ImageCache {
	...
	public Bitmap getBitmap(String url) {
		...
		if (bitmap == null) {
			 ...
			 */
			File cacheDir = new File(SmsDir.getInstance().getCache_dir());
			...
			if (cacheFiles != null) {
				...
				if (i < cacheFiles.length) {
					bitmap = getSDBitmap(SmsDir.getInstance().getCache_dir()
							+ "/" + fileName);
					/**
	...
	public void putBitmap(String url, Bitmap bitmap) {
		 ...
		 * 存到本地SD中
		 */
		putSDBitmap(url.substring(url.lastIndexOf("/") + 1, url.indexOf("?")),
				bitmap);
	}
	...
	private void putSDBitmap(final String fileName, final Bitmap bitmap) {
		new Thread(new Runnable() {
			...
			public void run() {
				...
				if (!cacheDir.exists())
					cacheDir.mkdirs();
				File cacheFile = new File(SmsDir.getInstance().getCache_dir()
						+ "/" + fileName);
				if (!cacheFile.exists()) {
					try {
