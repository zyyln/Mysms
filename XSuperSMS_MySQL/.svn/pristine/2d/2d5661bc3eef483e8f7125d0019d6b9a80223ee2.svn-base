...
public class as3992_native {
	...
	private static final String TAG = as3992_native.class.getSimpleName();
	...
	public byte[] search_card6B_helper(byte[] num) {
		...
		synchronized (lock) {
			...
			if (read_ok && (buf[0] > 0) && (buf.length >= 14)) {
				...
				Log.i(TAG, String.format("%02X%02X%02X%02X%02X%02X%02X%02X", uid[0], uid[1], uid[2], uid[3], uid[4],
						uid[5], uid[6], uid[7]));
				int freq = btoi(buf[11]) + (btoi(buf[12]) << 8) + (btoi(buf[13]) << 16);
				...
				if ((freq >= 920625) && (freq <= 924375)) {
					...
					for (int nn = 0; nn < tj.length; nn++) {
						if (nn == ids) {
							Log.w("as3992_native", "freq " + (920625 + nn * 750) + " times is " + tj[nn]);
						} else {
							Log.i("as3992_native", "freq " + (920625 + nn * 750) + " times is " + tj[nn]);
						}
					}
				}
				return uid;
			} else if (read_ok) {
				Log.e(TAG, "search 6B failed, read is " + read_ok + " num of tags is " + buf[0] + " buf length is "
						+ buf.length);
				read_ok = false;
				return null;
			} else {
	...
	public int write_tag6B(byte[] uid, int addr, byte[] cnt) {
		if ((uid == null) || (uid.length != 8) || (addr < 8) || (cnt.length > 243)) {
	...
	public int setlock(byte type, byte area, byte[] passwd) {
		...
		synchronized (lock) {
			...
			if (read_ok && (buf.length >= 1) && buf[0] == 0) {
				...
				if (buf.length >= 1) {
					Log.e("as3992", "lock card failed " + read_ok + " error " + buf[0]);
				} else {
	...
	public ArrayList<Tag_Data_Rssi> search_card_rssi() {
		...
		synchronized (lock) {
			do {
				...
				// if (read_ok && (buf[5] > 2) && (buf.length >= buf[5] + 6)) {
				// //8?
				if (read_ok && buf.length >= 6 && (buf[5] > 2) && (buf.length >= buf[5] + 6)) { // 8?
					...
					int ch_q = (buf[1] & 0xf);
					int freq = btoi(buf[2]) + (btoi(buf[3]) << 8) + (btoi(buf[4]) << 16);
					cx.add(new Tag_Data_Rssi(new Tag_Data(pc, epc), freq, ch_i, ch_q));
					card_num--;
				} else {
					read_ok = false;
					return null;
	...
	public int select_card(byte[] epc) {
		...
		synchronized (lock) {
			...
			if (read_ok && (buf.length >= 1) && buf[0] == 0) {
				...
				if (buf.length >= 1) {
					Log.e("as3992", "selsect card " + read_ok + " error " + buf[0]);
				} else {
	...
	public byte[] read_area(int area, int addr, int count) {
		...
		synchronized (lock) {
			...
			if (read_ok && (buf[1] > 0) && (buf.length > 1 ? (count == 0 ? true : buf[1] == count) : false)
					&& (count == 0 ? true : buf[0] == 0)) {
				read_ok = false;
				...
				if (buf.length >= 2) {
					Log.e("as3992", "read area failed " + read_ok + " error " + buf[0] + " readed number " + buf[1]);
				} else {
		...
		for (int i = 0; i < count;) {
			...
			synchronized (lock) {
				...
				if (read_ok && (buf.length >= 2) && buf[0] == 0) {
					...
					if (buf.length >= 2) {
						Log.e("as3992_native", "wrtie area failed " + read_ok + " error " + buf[0]
								+ " write ok : write number " + buf[1]);
					} else {
						Log.e("as3992_native", "wrtie area failed " + read_ok);
					}
					read_ok = false;
		...
		for (int freq = start; freq <= stop; freq += increment) {
			...
			synchronized (lock) {
				...
				if (read_ok && (buf.length >= 1) && buf[0] != 0) {
					...
					Log.e("as3992", "set freq failed or wait too long time " + freq);
	...
	public class Freq_Msg {
		Freq_Msg(int n_start, int n_stop, int n_inc, int n_rssi, int n_id, int n_listentime, int n_allocationtime,
				int n_idletime) {
			start = n_start;
			stop = n_stop;
			inc = n_inc;
	...
	public Freq_Msg get_freq() {
		...
		synchronized (lock) {
			...
			if (read_ok && (buf.length >= 18)) {
				...
				int start = btoi(buf[9]) + (btoi(buf[10]) << 8) + (btoi(buf[11]) << 16);
				int stop = btoi(buf[12]) + (btoi(buf[13]) << 8) + (btoi(buf[14]) << 16);
				int inc;
				if (btoi(buf[15]) == 1) {
					inc = 0;
				} else {
				...
				Log.i("as3992_natvie", "start " + start + " stop " + stop + " inc " + inc + " rssi " + rssi
						+ " listen " + listentime + " alloc " + allocationtime + " idle " + idletime);
				read_ok = false;
				return new Freq_Msg(start, stop, inc, rssi, id, listentime, allocationtime, idletime);
			} else {
				Log.e("as3992", "get freq failed " + read_ok + " buflength " + buf.length);
				read_ok = false;
				return null;
			}
		}
	...
	public short get_gen2(int which) {
		...
		synchronized (lock) {
			...
			if (read_ok && buf.length >= 12) {
				...
				Log.e("as3992", "get sens failed " + read_ok + " buf length " + buf.length);
				read_ok = false;
				return 1000;
			}
		}
	...
	public byte[] get_all_reg() {
		...
		synchronized (lock) {
			...
			if (read_ok && (buf.length >= 41)) {
				...
				Log.e("as3992", "get all reg " + read_ok + " buf length is " + buf.length);
				read_ok = false;
				return null;
			}
		}
	...
	public int[] get_freq_rssi_level(int freq) {
		...
		synchronized (lock) {
			...
			if (read_ok && (buf.length >= 3)) {
				...
				Log.e("as3992", "get freq rssi " + read_ok + " buf length is " + buf.length);
				read_ok = false;
				return null;
			}
		}
